...
static void fl_hw_destroy_filter(struct tcf_proto *tp, struct cls_fl_filter *f)
{
	...
	if (tc_can_offload(dev))
		__rh_call_ndo_setup_tc(dev, TC_SETUP_CLSFLOWER,
				       &cls_flower);
				...
				struct cls_fl_filter *f)
{
	...
	/*
	 * RHEL note: We need to preserve 'egress_dev' functionality for
	 * drivers compiled against RHEL-7.4 that uses it. In this case
	 * we have to call .ndo_setup_tc() callback of egress device
	 * associated with one of assigned actions.
	 *
	 * This emulation is done by handle_cls_flower_rh74() in
	 * net/core/rh_wrap.c to avoid RHEL specific code here.
	 */
	if (tc_can_offload(dev) || dev->netdev_ops->ndo_setup_tc_rh74) {
