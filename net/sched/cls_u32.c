				...
				u32 flags)
{
	...
	if (tc_can_offload(dev)) {
		err = __rh_call_ndo_setup_tc(dev, TC_SETUP_CLSU32,
					     &cls_u32);
		if (err) {
			if (skip_sw)
				return err;
...
{
	unsigned long size = U32_HASH_SIZE * sizeof(struct hlist_head);
	int i, ret;
...
#endif
	if (size > PAGE_SIZE)
		tc_u_common_hash = kmalloc(size, GFP_KERNEL);
	else
		tc_u_common_hash = vmalloc(size);
	if (!tc_u_common_hash)
		return -ENOMEM;
