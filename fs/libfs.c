...
static void move_cursor(struct dentry *cursor, struct list_head *after)
{
	...
	spin_lock(&parent->d_lock);
	__list_del(cursor->d_u.d_child.prev, cursor->d_u.d_child.next);
	if (after)
		list_add(&cursor->d_u.d_child, after);
	else
		list_add_tail(&cursor->d_u.d_child, &parent->d_subdirs);
	spin_unlock(&parent->d_lock);
}
...
loff_t dcache_dir_lseek(struct file *file, loff_t offset, int whence)
{
	...
	if (offset != file->f_pos) {
		...
		if (file->f_pos >= 2) {
			...
			move_cursor(cursor, to ? &to->d_u.d_child : NULL);
		}
	}
	mutex_unlock(&dentry->d_inode->i_mutex);
	return offset;
...
int dcache_readdir(struct file * filp, void * dirent, filldir_t filldir)
{
	...
	struct list_head *p = &cursor->d_u.d_child;
	struct dentry *next;
	bool moved = false;
	ino_t ino;
	int i = filp->f_pos;
